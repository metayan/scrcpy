...
public final class OpenGLRunner {
    ...
    private OpenGLRunner(OpenGLFilter filter, Size outputSize) {
        this.filter = filter;
    ...
    public static synchronized void initOnce() {
        if (handlerThread == null) {
            if (stopped) {
                throw new IllegalStateException("Could not init OpenGLRunner after it is stopped");
            }
    ...
    public static OpenGLRunner start(OpenGLFilter filter, Size inputSize, Size outputSize, Surface outputSurface)
            throws OpenGLException, InterruptedException {
        initOnce();

        OpenGLRunner runner = new OpenGLRunner(filter, outputSize);
        ...
        handler.post(() -> {
            try {
                runner.run(inputSize, outputSurface);
    ...
    private void run(Size inputSize, Surface outputSurface) throws OpenGLException {
    ...
    private void render() throws OpenGLException {
    ...
    public void stopAndRelease() throws InterruptedException {
        final Semaphore sem = new Semaphore(0);
